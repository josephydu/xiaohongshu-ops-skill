...
description: "小红书全链路运营技能：覆盖账号定位、选题研究、内容生产、发布、评论与复盘。Use when 用户想发/改/评 小红书笔记、需要小红书账号分析/选题/复刻/下载、或提到「小红书运营/发笔记/回评论/爆款复刻」。凡小红书浏览/搜索/发布/评论任务，默认使用 OpenClaw 内置浏览器 profile=\"openclaw\"。"
metadata:
  version: "2026.05"
---
...
按 `references/xhs-compliance-2026.md` 执行。

- 发布和评论前确认符合 2026 版平台规则
- AI 生成或辅助创作的内容发布时默认勾选「AI 辅助创作」
- 例外：用户明确声明该篇为"全部人工创作"时可不勾选，但需用户确认
- 违禁词与风控信号处置见该文件
...
按 `references/xhs-target-download.md` 执行。

- 输入：一条小红书笔记 URL
- 输出：正文、图片、元信息的结构化下载产物
- 用于复刻前拆解或归档；不做批量爬取
